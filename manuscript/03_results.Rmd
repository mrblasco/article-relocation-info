...
net_senders <- filter(ds_asylum_rel, country_type == "Net senders")$country
...
ds_plot %>%
    ...
    facet_grid(
        ifelse(is.na(country_type), "All", country_type) ~ relocation,
        scales = "free_y",
        space = "free"
    ) + 
    geom_vline(
    ...
    scale_shape_manual(
        ...
        values = c(1, 2),
        labels = c(
            absolute = "Absolute vs. No information",
            relative = "Relative vs. No information"
        )
    ) +
    scale_color_brewer(
        name = NULL,
    ...
    scale_y_discrete(
        name = NULL, 
        labels = function(x) gsub("relocation_treatment", "", x)
    ) +
    scale_x_continuous(
        name = "Mean difference in fairness rank",
        limits = c(-1, 1) * 0.7
    ) + 
    theme(
