...
In this study, GDP and population are used as proxies for “capacity,” consistent with existing EU allocation proposals and policy debates [@thielemann2018refugee]. These indicators also provide respondents with clear and easily interpretable signals of a country’s size and wealth. Research shows that individuals commonly rely on such measures when evaluating “fairness” in international sharing [@kleider2019politics]. We acknowledge, however, that respondents may interpret these indicators in different ways (e.g. housing capacity or labour market conditions). As a result, relying on these measures may conflate different understandings of “capacity” into a single dimension, which may limit the interpretation of the results.


To illustrate the main logic behind the experiment, we present a simplified scenario involving only two countries, Poland and Greece. While Poland has a larger population, Greece has handled more than double the average number of asylum applications over the past ten years (2014 - 2023). Under a system of proportional relocation, Poland will experience more asylum applications, than under no relocation. We will call such countries “net receivers”. The opposite would hold true for Greece where any form of relocation will result in a lower number of applications under any of the relocation scenarios. We call such countries “net senders”. 
...
tables$raw$tbl_example %>%
    kableExtra::kbl(
        col.names = c("", "Abs.", "Rel.", "Abs.", "Rel.", ""),
        caption = "Illustrative example for two countries", 
        booktabs = TRUE, 
        align = "lrrrrr",
        linesep = "\\addlinespace\\hline\\addlinespace",
    ) %>%
    kableExtra::kable_styling(full_width = TRUE) %>%
    kableExtra::column_spec(1, italic = TRUE, width = "10em")  %>% 
    kableExtra::pack_rows("Hypothetical situation:", 1, 3) %>%
    kableExtra::pack_rows("Information treatment:", 4, 6) %>%
    kableExtra::add_header_above(c("", "Country A" = 2, "Country B" = 2, "All"))
```
...
```{r asylum-per-capita}
tables$raw$tbl_asylum_rel %>%
    dplyr::mutate(
        diff_gdp = no_relocation - gdp,
        diff_pop = no_relocation - population
    ) %>%
    dplyr::arrange(desc(diff_gdp)) %>%
    dplyr::select(
      Country = country,
    ...
    kableExtra::kbl(
        caption = "Expected asylum seekers per 100 000 residents, under different allocation rules",
        booktabs = TRUE,
        linesep = "",
    ) %>% 
    kableExtra::kable_styling(full_width = TRUE)
...
```{r summary}
tbl_covariates <- tables$raw$tbl_covariates
tbl_covariates %>%
    dplyr::mutate(
        variable = gsub("Importance of", "Value of", variable) %>% 
            gsub("Perceived evenness of AS distribution in EU", "Evenness of AS in EU", .)
    ) %>% 
    dplyr::select(
        Variable = variable,
        "Control" = "(Intercept)",
        "Diff. Rel." = "relocation_treatmentRelative",
        "Diff. Abs." = "relocation_treatmentAbsolute",
    ) |>
    kableExtra::kbl(
        booktabs = TRUE, 
        digits = 1,
        caption = "Summary statistics (standardized mean difference in square brackets)"
    ) |>
    kableExtra::kable_styling(full_width = TRUE) |>
    ...
    kableExtra::pack_rows("Other variables", 1 + 5 + 4, nrow(tbl_covariates)) 
```
...
## Fair share and political leanings 
